...
### 4️⃣ **Immer Patches - 协同编辑的行业标准** ⭐

> **革命性能力：Immer Patches 天然支持协同编辑**
...
class DocStore extends ZenithStore<DocState> {
  ...
  applyRemote(patches: Patch[]) {
    this.produceData(
      state => applyPatches(state, patches),  // Immer 的 applyPatches
      true  // 远程操作不记录到本地历史
    )
  }
}
...
| 维度 | 传统方案（全量同步） | Zenith + Immer Patches |
|------|---------------------|------------------------|
| **带宽消耗** | 1MB 文档 = 1MB 传输 | 1MB 文档 ≈ 1KB Patches |
| **冲突解决** | 复杂（需要 CRDT） | 简单（基于操作序列） |
| **历史回溯** | 需单独实现 | 内置支持（inversePatches） |
| **OT 算法兼容** | 需自己设计格式 | JSON Patch 标准格式 |
...
**为什么选择 Immer？**
- ✅ 业界标准的 JSON Patch 格式
- ✅ 与 OT（Operational Transform）算法天然兼容
- ✅ 支持 CRDT（Conflict-free Replicated Data Types）集成
- ✅ 完善的序列化/反序列化能力

---

## 🏗️ 技术架构：基于 Immer 的深度集成

```typescript
class ZenithStore<T> {
  // 核心方法：基于 Immer 的 produce
  produceData(fn: (draft: T) => void, disableRecord?, patchCallback?) {
    const prevState = this._state
    
    // 🔥 Immer 的魔法：自动追踪变化 + 生成 Patches
    const newState = produce(
      this._state, 
      fn,  // 你的更新逻辑（mutable 风格）
      (patches, inversePatches) => {
        // Immer 自动生成的 Patches
    ...
    this._state = newState  // Immer 保证的不可变更新
    this._listeners.forEach((listener) => listener(newState, prevState))
  }
}
```

**Immer 为 Zenith 提供的核心能力**：

1. **不可变更新** - `produce` 自动生成新的不可变状态
2. **Patches 追踪** - 自动记录所有变化为 JSON Patch 格式
3. **简洁的更新语法** - Mutable 风格写法，Immutable 更新保证
4. **性能优化** - 结构共享（Structural Sharing）
